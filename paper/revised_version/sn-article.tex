...
\emph{graph-free baseline} (manuscript name T-GCN-NoSpatial / GCN-NoSpatial);
...
RMSE, while the graph-free baseline obtains $5.25\pm 0.19$ --- a $33.3\%$ relative
reduction when the physical graph is removed. the graph-free baseline beats Physical in
...
about $11\%$ worse than the graph-free baseline. Neither contemporaneous graph beats
the graph-free baseline in any dataset$\times$horizon cell. For GCN, cGSL is better than
directed GSL on Los-loop ($5.76$ vs.\ $7.83$) but still worse than
the graph-free baseline; for T-GCN, GSL and cGSL are nearly indistinguishable.
...
\begin{table}[t]
\centering
\caption{Edge-budget sweep on Los-loop PH1 (five-seed mean $\pm$ sample SD; \emph{lower RMSE is better}). RandTopK and CorrTopK are T-GCN with $K$ random or top-correlation directed edges. The italic first row is the graph-free control. The last two rows are Stage~40 MultiGSL/Mix at the canonical $K=30$ budget. $\Delta_{\mathrm{rel}}$ is the relative RMSE difference vs the graph-free baseline (positive = higher error than the baseline; negative = lower error). Bold marks Mix, the lowest mean in the table.}
...
\midrule
T-GCN-MultiGSL ($K{=}30$, Stage~40) & $4.84\pm 0.11$ & $-7.5\%$ \\
T-GCN-MultiGSL-Mix ($K{=}30$, Stage~40) & \textbf{$4.49\pm 0.14$} & $-14.2\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
degrade more slowly; neither family reaches the Stage~40 MultiGSL/Mix
...
with the graph-free baseline and T-GCN-MultiGSL-Mix (normalized scale).}
\label{fig:predvsact}
\end{figure}
...
Figure~\ref{fig:predvsact-sz} shows an analogous predicted-versus-actual overlay on SZ-Taxi: the graph-free baseline and Mix almost coincide, matching the aggregate results.
...
(and in the main Stage~40 matrix). There is no sparsified physical graph,
