...
Figure~\ref{fig:rmsebars} summarizes five-seed mean RMSE for key Los-loop
PH1 configurations, split by family. Figure~\ref{fig:graphstruct}
contrasts the dense physical adjacency with the sparser multi-lag union.

\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/rmse_comparison_los_ph1.pdf}
\caption{Los-loop PH1: five-seed mean RMSE (whiskers: sample SD), split by
family. (a) T-GCN; (b) GCN. Dashed lines mark the corresponding
NoSpatial control.}
\label{fig:rmsebars}
\end{figure}
...
Table~\ref{tab:controls} and Figure~\ref{fig:sparsity} report the
matched-sparsity control. At a $30$-edge budget, RandTop30 ($6.10\pm 0.12$)
and CorrTop30 ($5.39\pm 0.10$) are worse than NoSpatial ($5.25\pm 0.19$),
while DAGMA multi-lag graphs with lag-aligned use are better. Sparsity
alone does not explain the multi-lag gain on this cell (Los-loop PH1 only).
A capacity-matched graph-free model (hidden $74$; single seed) stays near
$5.14$ RMSE.
